-- MARKETLENS GOLD LAYER: turns cleaned Silver data into ready-to-query tables for the dashboard and AI agent
-- Reads from: silver.silver_market_data, silver.silver_symbol_metadata
-- One procedure does everything: gold.sp_refresh_all(run_id)
-- run_id is just a label stamped on audit rows so we can trace one run later - it never filters data
-- Every step below writes its own SUCCESS/FAILED row to the shared audit table, same one Bronze and Silver use
-- If any step fails, the whole procedure stops - a broken step can never be silently ignored

CREATE OR REPLACE PROCEDURE
`market-lens-506611.gold.sp_refresh_all`(p_run_id STRING)
BEGIN

    -- reused as a stopwatch before every step, so each step's audit row knows how long it took
    DECLARE v_step_started_at TIMESTAMP;

    -- how many days back we recheck when updating fact_daily_metrics, sized to cover our longest rolling average (200 days)
    DECLARE v_ma_lookback_days INT64 DEFAULT 300;

    -- how many rows the fact_daily_metrics update actually touched this run, for an honest audit number
    DECLARE v_rows_affected INT64;

    -- STEP 0: get one clean, deduplicated copy of Silver's data ready to use - scratch table, gone once this run ends
    ...
    QUALIFY ROW_NUMBER() OVER (
        ...
        ORDER BY silver_loaded_at DESC, Date DESC  -- most recently written row wins, not whichever run's ID sorts higher alphabetically
    ) = 1;
    ...
    -- STEP 1: dim_security - one row per ticker, its name/exchange/listing tier, rebuilt fresh every run since it's cheap
    ...
    BEGIN
        ...
        QUALIFY ROW_NUMBER() OVER (
            PARTITION BY symbol
            ORDER BY symbol
        ) = 1;                            -- just a safety net in case a symbol ever shows up twice
    ...
    -- STEP 2: fact_daily_metrics - one row per stock per day, the engine everything else is built from; updated incrementally, not rebuilt from scratch
    SET v_step_started_at = CURRENT_TIMESTAMP();

    BEGIN

        -- create it once with an exact schema; if it already exists, this line does nothing
        ...
        (
            ...
            asset_type STRING,
            Close FLOAT64,
            Volume FLOAT64,   -- a decimal, matching how Silver actually stores it
        ...
        -- update if the row exists, insert if it's new - this MERGE is what makes re-running safe
        MERGE `market-lens-506611.gold.fact_daily_metrics` T
        USING (

            -- for each symbol, find the last day we already have - NULL means a brand new symbol
            ...
            -- only look at new symbols, or recent days within our lookback window - not full history
            recompute_scope AS (
                SELECT g.*
                FROM gold_base g
            ...
            SELECT
                ...
                SAFE_DIVIDE(Volume - rolling_avg_volume_30d, rolling_stddev_volume_30d) AS volume_zscore,  -- how unusual today's volume is vs this stock's own normal
                rolling_max_close,
                SAFE_DIVIDE(Close - rolling_max_close, rolling_max_close) AS drawdown_pct,  -- how far below its all-time high, right now
        ...
        WHEN MATCHED THEN
            -- already have this row - just refresh its values
            UPDATE SET
                Close = S.Close,
                Volume = S.Volume,
        ...
        SET v_rows_affected = @@row_count;  -- honest count of what this run touched, not the whole table's size
    ...
    -- STEP 3: fact_period_returns - for every stock, its % gain or loss over 1 week/1 month/3 months/1 year/3 years
    SET v_step_started_at = CURRENT_TIMESTAMP();
    ...
    BEGIN
        ...
        WITH latest_date_per_symbol AS (
            SELECT symbol, MAX(Date) AS latest_date  -- each symbol's most recent trading day, our "today" anchor
            FROM gold_base
            GROUP BY symbol
        ),
        period_definitions AS (
            -- one row per symbol per window, with the date we're trying to find a matching price for
        ...
        closest_start_prices AS (
            -- picks the real trading day closest to each window's theoretical start (markets are shut on weekends/holidays)
            SELECT
        ...
        end_prices AS (
            SELECT f.symbol, f.Date AS end_date, f.Close AS end_close  -- today's actual closing price
            FROM gold_base f
        ...
        SELECT
            ...
            SAFE_DIVIDE(e.end_close - s.start_close, s.start_close) * 100 AS return_pct  -- the actual % change
        FROM closest_start_prices s
        JOIN end_prices e ON s.symbol = e.symbol
        WHERE s.closeness_rank = 1;   -- keep only the single best-matched start date per symbol+window
    ...
    -- STEP 4: mart_screener - the main table the dashboard queries most, one row per stock with everything joined in already
    SET v_step_started_at = CURRENT_TIMESTAMP();
    ...
    BEGIN
        ...
        WITH latest_metrics AS (
            SELECT *
            FROM `market-lens-506611.gold.fact_daily_metrics`
            QUALIFY ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY Date DESC) = 1  -- just each symbol's most recent row
        )
        SELECT
        ...
        -- filters out stocks whose return looks like a data glitch, not a real trading outcome (see top-of-file note)
    ...
    -- STEP 5: mart_normalized_prices - every stock rescaled to start at 100, so a $5 stock and a $500 stock are fairly comparable
    SET v_step_started_at = CURRENT_TIMESTAMP();
    ...
    BEGIN
        ...
        AS
        WITH base_prices AS (
            SELECT symbol, MIN(Date) AS base_date  -- each stock's very first trading day, its "100" point
            FROM gold_base
            GROUP BY symbol
        ),
        indexed AS (
            SELECT
                ...
                SAFE_DIVIDE(f.Close, base_close.Close) * 100 AS indexed_price  -- today's price as a % of the starting price
            FROM gold_base f
        ...
        SELECT *
        FROM indexed
        WHERE indexed_price BETWEEN 1 AND 100000;  -- same data-glitch safety filter as mart_screener
    ...
    -- STEP 6: mart_unusual_volume - just the stocks trading abnormally today, answers "unusual volume spikes" directly
    SET v_step_started_at = CURRENT_TIMESTAMP();
    ...
    BEGIN
        ...
        FROM `market-lens-506611.gold.fact_daily_metrics`
        WHERE ABS(volume_zscore) > 2   -- more than 2 standard deviations from this stock's own normal volume
    ...
    -- VIEWS: saved queries, not real tables - always show live data from the tables above, no storage of their own

    -- v_top_returns: just the return columns a "top performers" chart needs, not all 19 columns of mart_screener
    SET v_step_started_at = CURRENT_TIMESTAMP();
    BEGIN
    ...
    -- v_normalized_price_comparison: a simple window onto mart_normalized_prices, for charting price paths over time
    SET v_step_started_at = CURRENT_TIMESTAMP();
    BEGIN
    ...
    -- STEP 7: mart_sector_summary + v_sector_comparison - grouped by Nasdaq listing tier (Q/G/S), NOT real industry sector - no sector data exists in our source
    SET v_step_started_at = CURRENT_TIMESTAMP();
    ...
    BEGIN
        ...
        AS
        SELECT
            COALESCE(NULLIF(TRIM(market_category), ''), 'O') AS market_category,  
        ...
        FROM `market-lens-506611.gold.mart_screener`
        GROUP BY COALESCE(NULLIF(TRIM(market_category), ''), 'O');
    ...
    -- v_sector_comparison: one stock's own 1-year return next to its listing tier's average - label this "listing tier" on the dashboard, never "sector"
    SET v_step_started_at = CURRENT_TIMESTAMP();
    BEGIN
